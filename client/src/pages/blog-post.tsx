...
export default function BlogPostPage() {
  ...
  return <div className="min-h-screen">
      <Helmet>
        ...
        {post.coverImage && <meta property="og:image" content={`https://amon-tour.com${post.coverImage}`} />}
        
        {/* Twitter Card Meta Tags */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${post.title} | Amon Tour`} />
        <meta name="twitter:description" content={post.metaDescription || post.excerpt} />
        {post.coverImage && <meta name="twitter:image" content={`https://amon-tour.com${post.coverImage}`} />}
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-4xl mx-auto">
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
              ...
              {post.coverImage && <div className="relative h-64 md:h-96 overflow-hidden">
                  <img src={post.coverImage} alt={post.imageAltText || `Krabi Guide - ${post.title} | Amon Tour`} title={post.title} className="w-full h-full object-cover transition-transform duration-300 hover:scale-105" loading="eager" fetchPriority="high" onError={e => {
                e.currentTarget.style.display = 'none';
              }} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent"></div>
                  {post.category && <Badge className="absolute top-4 left-4 bg-primary shadow-lg">
                      {post.category.name}
                    </Badge>}
                </div>}
              ...
              <div className="p-8">
                ...
                {post.tags && post.tags.length > 0 && <div className="flex flex-wrap gap-2 mb-8">
                    {post.tags.map((tag: any) => <Badge key={tag.id} variant="secondary">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm">
                                ...
                                <span className="flex items-center gap-1 text-[hsl(var(--success))]">
                                  <span>💰</span>
                                  <strong>
                                    {tour.currency === 'THB' ? `${tour.price} THB (~${Math.round(tour.price / 36)}€)` : `From ${tour.price}€`}
                                  </strong>
                                </span>
                              </div>
                            </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {(relatedPosts as any[]).slice(0, 2).map((relatedPost: any) => <Card key={relatedPost.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                      {relatedPost.coverImage && <div className="relative h-48">
                          <img src={relatedPost.coverImage} alt={relatedPost.title} className="w-full h-full object-cover" onError={e => {
                    e.currentTarget.style.display = 'none';
                  }} />
                        </div>}
                      <CardHeader>
