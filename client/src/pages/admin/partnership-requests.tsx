...
import { ArrowLeft, Handshake, Eye, Trash2, Mail, CheckCircle2, Building, Globe, Briefcase } from "lucide-react";
...
export default function AdminPartnershipRequests() {
  ...
  } = useQuery<PartnershipRequest[]>({
    queryKey: ["/api/partnership-requests", showUnreadOnly ? {
      read: false
    } : {}],
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (id: number) => {
      await apiRequest("PATCH", `/api/partnership-requests/${id}`, {
        read: true
      });
    },
    onSuccess: () => {
                      ...
                      {requests.map(request => <TableRow key={request.id}>
                          ...
                          <TableCell>
                            {request.website ? <a href={request.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                                {request.website}
                              </a> : <span className="text-gray-400">{t("Nonrenseignxe9", {
                          defaultValue: "Nonrenseignxe9"
                        })}</span>}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <Button variant="outline" size="sm" onClick={() => window.open(`mailto:${request.email}?subject=Votre demande de partenariat`)}>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedRequest && <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">{t('Nom de l\'entreprise', {
                ...
                {selectedRequest.website && <div className="col-span-2">
                    <label className="text-sm font-medium text-gray-700">{t("Siteweb", {
                  defaultValue: "Siteweb"
                })}</label>
