...
export default function BookingIframe() {
  ...
  return <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow flex flex-col">
        {bookingLink && <motion.div className="flex-grow w-full bg-white" initial={{
        opacity: 0
      }} animate={{
        ...
        duration: 0.5
            ...
            {isLoading && <div className="flex justify-center items-center h-[calc(100vh-80px)]">
                <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
              </div>}
            <iframe src={bookingLink} title={`Booking for ${title}`} className="w-full h-full border-0" style={{
          height: 'calc(100vh - 80px)',
          display: isLoading ? 'none' : 'block'
        }} onLoad={handleIframeLoad} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
          </motion.div>}
      </main>
