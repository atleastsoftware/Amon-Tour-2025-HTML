  ...
  return <div className="flex flex-col">
      ...
      y: -5
        ...
        if (customLink && customLink.includes('tourninja.io')) {
          ...
          <div className="relative aspect-video overflow-hidden cursor-pointer" onClick={e => {
          ...
          if (isMobileDevice) {
            ...
            {images && images.length > 0 ? <img src={images[0]} alt={title} className="w-full h-full object-cover transition-transform duration-500 hover:scale-105" /> : <div className="w-full h-full bg-gray-200 flex items-center justify-center">
                <span className="text-gray-400">{t("Noimage", {
          ...
          <CardContent className="flex flex-col flex-grow p-5">
            ...
            {tags && tags.length > 0 && <div className="flex flex-wrap gap-1 mb-3">
                {tags.map((tag, index) => <Badge key={index} variant="outline" className="text-xs bg-gray-50">
                    {tag}
                  </Badge>)}
              </div>}
      ...
      <AnimatePresence>
        {isBookingOpen && <motion.div className="fixed inset-0 z-50 bg-white md:relative md:z-auto md:w-full md:my-6 md:border-2 md:border-primary/20 md:rounded-xl md:shadow-xl md:overflow-hidden" initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        ...
        ease: "easeInOut"
          ...
          minHeight: '500px'
              ...
              <iframe src={customLink} title={`Booking for ${title}`} className="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
            </div>
          </motion.div>}
      </AnimatePresence>
