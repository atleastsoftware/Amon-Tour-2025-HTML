...
- Pack client (Task « Pack client prompts et liens MCP ») : `docs/PACK-CLIENT-MCP.md` (guide complet, URL de production `https://www.amon-tour.com/mcp`) et `docs/PACK-CLIENT-MCP-AIDE-MEMOIRE.md` (1 page). Le MCP en production ne répond qu'après republication + secrets `MCP_AUTH_TOKEN`, `GITHUB_TOKEN`, `GITHUB_REPO`.
