...
- October 16, 2025: Fixed critical WYSIWYG editor issues - resolved 7 major problems affecting page editor functionality: (1) Removed debug console.log and visual debug banner from SearchBarToursBlock; (2) Enhanced Hero block with gradient background support (backgroundType: 'gradient') and conditional animations (hasAnimation property); (3) Fixed PopularExperiencesBlock to correctly extract tour data from API response wrapper {success, data}; (4) Corrected both Card Grid Date (popular_experiences) and Card Grid Price (tour_ninja_section) blocks to display real Tour Ninja tours instead of fallback data; (5) Verified BlogSearchBlock already correctly displays only existing categories/tags from database; (6) Confirmed DynamicFormBlock loads configuration from dedicated form editor; (7) Added missing preview case for Text + Images block (who_we_are) in BlockPreview.tsx to enable visibility in editor. All editor modifications now render identically on public site (true WYSIWYG).
