...
- **Programmatic Destination Pages**: `server/ssrDestinationRoutes.ts` adds 7 keyword-targeted SSR landing pages (`/destinations` hub + 6 individual pages: `/destinations/agence-francophone-krabi`, `/destinations/koh-phi-phi`, `/destinations/phang-nga-bay`, `/destinations/thalane-bivouac`, `/destinations/catamaran-krabi`, `/destinations/railay-beach`). Each page has 400+ words of real content, TouristTrip + FAQPage JSON-LD, WhatsApp CTAs, and internal links. All 7 URLs are in the sitemap at priority 0.8–0.9. All routes are universal SSR (no bot-gating) since these pages have no SPA counterpart.
